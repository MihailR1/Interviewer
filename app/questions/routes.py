...
async def all_questions(number_of_questions: int = 15, is_random: bool = True) -> list[Question]:
    questions: list[Question] = await QuestionCRUD.select_questions_with_status(
        status=Status.active
    )

    if is_random:
        choice_random_questions = random.choices(questions, k=number_of_questions)
...
async def create_question(
    ...
    exist_question: Question | None = await QuestionCRUD.find_question_by_same_title_or_text(
    ...
    save: Question = await QuestionCRUD.insert(user_id=current_user.id, **question.model_dump())
...
async def count_statistics_for_questions(question_id: int, stat_name: StatsCount) -> None:
    select_question: Question | None = await QuestionCRUD.select_by_id_or_none(id=question_id)
